...
export default function AvatarEditor() {
  ...
  const [frameUrl] = useState<string>("/Khung MohinhViet.png"); // Frame nằm trong public folder
  ...
  const onMove = (e: PointerEvent) => {
    ...
    setAvatar((prev) => ({ ...prev, x: dragStart.current.x + dx, y: dragStart.current.y + dy }));
  };
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-900 text-white mt-[80px]">
      ...
      <aside className="w-full md:w-80 p-4 bg-slate-800 space-y-4">
        <div>
          <h3 className="text-sm font-bold uppercase text-gray-400 mb-2">Ảnh khung</h3>
          <p className="text-gray-400">Khung ảnh đã được cố định.</p>
        </div>

        <div>
          <h3 className="text-sm font-bold uppercase text-gray-400 mb-2">Ảnh avatar</h3>
          <input
            type="file"
            accept="image/*"
            onChange={(e) => {
          ...
          <input
            ...
            step="0.01"
            value={avatar.scale}
            onChange={(e) => setAvatar({ ...avatar, scale: parseFloat(e.target.value) })}
          />
          ...
          <input
            ...
            step="1"
            value={avatar.rotDeg}
            onChange={(e) => setAvatar({ ...avatar, rotDeg: parseFloat(e.target.value) })}
          />
        </div>

        <div>
          <h3 className="text-sm font-bold uppercase text-gray-400 mb-2">Xuất ảnh</h3>
          <button className="w-full bg-cyan-600 py-2 rounded mb-2" onClick={exportImage}>
            Tải ảnh
          </button>
          <button className="w-full bg-slate-700 py-2 rounded" onClick={resetAll}>
            Reset
          </button>
        </div>
      </aside>
        ...
        >
          {avatarUrl && (
            <img
              ...
              src={avatarUrl}
              alt="Avatar"
              className="absolute top-0 left-0 select-none touch-none cursor-grab"
